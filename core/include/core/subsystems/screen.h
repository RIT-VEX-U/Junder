...
class SliderWidget {
  public:
    ...
    SliderWidget(double &val, double low, double high, Rect rect,
                 std::string name)
...
class WidgetPage : public Page {
  public:
    ...
    void draw(vex::brain::lcd &, bool /* first_draw */,
...
/**
 * @brief Start the screen background task. Once you start this, no need to draw
 * to the screen manually elsewhere
...
/// @brief stops the screen. If you have a drive team that hates fun call this
/// at the start of opcontrol
void stop_screen();
...
using draw_func_t =
    std::function<void(vex::brain::lcd &screen, bool, unsigned int)>;
...
class StatsPage : public Page {
  ...
  private:
    void draw_motor_stats(const std::string &name, vex::motor &mot,
...
class OdometryPage : public Page {
  public:
    ...
    OdometryPage(OdometryBase &odom, double robot_width, double robot_height,
                 bool do_trail);
    /// @brief @see Page#update
    void update(bool was_pressed, int x, int y) override;
    /// @brief @see Page#draw
...
class PIDPage : public Page {
  public:
    ...
    /// @param name a name to recognize this pid controller if we've got
    /// multiple pid screens
