 ...
 *
 * This class defines a top-level motion profile, which can act as an
 * intermediate between a subsystem class and the motors themselves
 *
 * This takes the constants kS, kV, kA, kP, kI, kD, max_v and acceleration and
 * wraps around a feedforward, PID and trapezoid profile. It does so with the
 * following formula:
 *
 * out = feedfoward.calculate(motion_profile.get(time_s)) +
 * pid.get(motion_profile.get(time_s))
 *
 * For PID and Feedforward specific formulae, see pid.h, feedforward.h, and
 * trapezoid_profile.h
 *
 * @author Ryan McGee
 * @date 7/13/2022
...
class MotionController : public Feedback {
  public:
     ...
     * When motion pofile is given a target to drive to, max_v and accel are
     * used to make the trapezoid profile instructing the controller how to
     * drive pid_cfg, ff_cfg are used to find the motor outputs necessary to
     * execute this path
     */
    typedef struct {
        double max_v;              ///< the maximum velocity the robot can drive
        double accel;              ///< the most acceleration the robot can do
        PID::pid_config_t pid_cfg; ///< configuration parameters for the
                                   ///< internal PID controller
        FeedForward::ff_config_t
            ff_cfg; ///< configuration parameters for the internal
    } m_profile_cfg_t;
     ...
     */
    void init(double start_pt, double end_pt, double start_vel,
     ...
     * should be applied. if limits are applied, the controller will not target
     * any value below lower or above upper
     *
    ...
    /**
     * This method attempts to characterize the robot's drivetrain and
     * automatically tune the feedforward. It does this by first calculating the
     * kS (voltage to overcome static friction) by slowly increasing the voltage
     * until it moves.
     *
     * Next is kV (voltage to sustain a certain velocity), where the robot will
     * record it's steady-state velocity at 'pct' speed.
     *
     * Finally, kA (voltage needed to accelerate by a certain rate), where the
     * robot will record the entire movement's velocity and acceleration, record
     * a plot of [X=(pct-kV*V-kS), Y=(Acceleration)] along the movement, and
     * since kA*Accel = pct-kV*V-kS, the reciprocal of the linear regression is
     * the kA value.
     *
