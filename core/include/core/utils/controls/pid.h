...
public:
  ...
  pid_config_t
      &config; ///< configuration struct for this controller. see pid_config_t
               ///< for information about what this contains

private:
  double last_error =
      0; ///< the error measured on the last iteration of update()
  double accum_error =
      0; ///< the integral of error over time since we called init()

  double last_time = 0; ///< the time measured the last time update() was called
  double on_target_last_time =
      0; ///< the time at which we started being on target

  double lower_limit =
      0; ///< the PID controller will never set a target to go lower than this
  double upper_limit =
      0; ///< the PID controller will never set a target to go higher than this
