...
class TankDrive {
  public:
    ...
    AutoCommand *DriveToPointCmd(point_t pt,
                                 vex::directionType dir = vex::forward,
                                 double max_speed = 1.0,
                                 double end_speed = 0.0);
    AutoCommand *DriveToPointCmd(Feedback &fb, point_t pt,
                                 vex::directionType dir = vex::forward,
                                 double max_speed = 1.0,
                                 double end_speed = 0.0);

    AutoCommand *DriveForwardCmd(double dist,
                                 vex::directionType dir = vex::forward,
                                 double max_speed = 1.0,
                                 double end_speed = 0.0);
    AutoCommand *DriveForwardCmd(Feedback &fb, double dist,
                                 vex::directionType dir = vex::forward,
                                 double max_speed = 1.0,
                                 double end_speed = 0.0);
    ...
    AutoCommand *TurnToHeadingCmd(double heading, double max_speed = 1.0,
                                  double end_speed = 0.0);
    AutoCommand *TurnToHeadingCmd(Feedback &fb, double heading,
                                  double max_speed = 1.0,
                                  double end_speed = 0.0);

    AutoCommand *
    TurnToPointCmd(double x, double y,
    ...
    AutoCommand *PurePursuitCmd(PurePursuit::Path path, directionType dir,
     ...
     */
    bool drive_forward(double inches, directionType dir, Feedback &feedback,
                       double max_speed = 1, double end_speed = 0);
    ...
    bool drive_to_point(double x, double y, vex::directionType dir,
                        Feedback &feedback, double max_speed = 1,
                        double end_speed = 0);
     ...
     * velocity by its completion
     */
    bool turn_to_heading(double heading_deg, Feedback &feedback,
                         double max_speed = 1, double end_speed = 0);
    /**
     * Turn the robot in place to an exact heading relative to the field.
     ...
     */
    bool pure_pursuit(PurePursuit::Path path, directionType dir,
                      Feedback &feedback, double max_speed = 1,
                      double end_speed = 0);
  ...
  private:
    ...
    PID correction_pid; ///< PID controller used to drive in as straight a line
                        ///< as possible
    Feedback *drive_default_feedback =
        NULL; ///< feedback to use to drive if none is specified
    Feedback *turn_default_feedback =
    ...
    bool func_initialized =
        false; ///< used to control initialization of autonomous driving. (you
               ///< only wan't to set the target once, not every iteration that
               ///< you're driving)
    bool is_pure_pursuit =
        false; ///< true if we are driving with a pure pursuit system
};
