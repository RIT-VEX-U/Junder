...
TakeBackHalf::TakeBackHalf(double gain, double first_cross_split, double thresh) : TBH_gain(gain), first_cross_split(first_cross_split), on_target_threshhold(fabs(thresh))
{
    tbh = 0.0;
    output = 0.0;
    prev_error = 0.0;
...
{
    return fabs(prev_error) < on_target_threshhold;
}
