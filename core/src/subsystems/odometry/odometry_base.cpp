...
double OdometryBase::pos_diff(pose_t start_pos, pose_t end_pos) {
    ...
    double retval =
        sqrt(pow(end_pos.x - start_pos.x, 2) + pow(end_pos.y - start_pos.y, 2));
 ...
 * Returns the difference between -180 degrees and +180 degrees, representing
 * the robot turning left or right, respectively.
 */
