...
AutoCommand *TankDrive::DriveToPointCmd(point_t pt, vex::directionType dir,
                                        double max_speed, double end_speed) {
    return new DriveToPointCommand(*this, *drive_default_feedback, pt, dir,
                                   max_speed, end_speed);
}

AutoCommand *TankDrive::DriveForwardCmd(double dist, vex::directionType dir,
                                        double max_speed, double end_speed) {
    return new DriveForwardCommand(*this, *drive_default_feedback, dist, dir,
                                   max_speed, end_speed);
}
...
AutoCommand *TankDrive::TurnToHeadingCmd(double heading, double max_speed,
                                         double end_speed) {
    return new TurnToHeadingCommand(*this, *turn_default_feedback, heading,
                                    max_speed, end_speed);
}
...
AutoCommand *TankDrive::TurnDegreesCmd(double degrees, double max_speed,
                                       double end_speed) {
    return new TurnDegreesCommand(*this, *turn_default_feedback, degrees,
                                  max_speed, end_speed);
}
...
AutoCommand *TankDrive::PurePursuitCmd(PurePursuit::Path path,
                                       directionType dir, double max_speed,
                                       double end_speed) {
    return new PurePursuitCommand(*this, *drive_default_feedback, path, dir,
                                  max_speed, end_speed);
}
AutoCommand *TankDrive::PurePursuitCmd(Feedback &feedback,
                                       ...
                                       directionType dir, double max_speed,
                                       double end_speed) {
    return new PurePursuitCommand(*this, feedback, path, dir, max_speed,
                                  end_speed);
}

AutoCommand *TankDrive::DriveStalledCmd() {
    class DriveStalledCommand : public AutoCommand {
      public:
        DriveStalledCommand(TankDrive &td) : td(td) {}
        bool run() override { return td.odometry->get_speed() < 1; }
        TankDrive &td;
    };
    return new DriveStalledCommand(*this);
}
