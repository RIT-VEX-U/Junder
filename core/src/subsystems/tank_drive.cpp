...
TankDrive::TankDrive(motor_group &left_motors, motor_group &right_motors,
                     robot_specs_t &config, OdometryBase *odom)
...
AutoCommand TankDrive::DriveToPointCmd(Feedback &fb, point_t pt,
                                       vex::directionType dir, double max_speed,
                                       double end_speed) {
    return DriveToPointCommand(*this, fb, pt, dir, max_speed, end_speed);
}

AutoCommand TankDrive::DriveToPointCmd(point_t pt, vex::directionType dir,
                                       double max_speed, double end_speed) {
    return DriveToPointCommand(*this, *drive_default_feedback, pt, dir,
                                   max_speed, end_speed);
}

AutoCommand TankDrive::DriveForwardCmd(double dist, vex::directionType dir,
                                       double max_speed, double end_speed) {
    return DriveForwardCommand(*this, *drive_default_feedback, dist, dir,
                               max_speed, end_speed);
}

AutoCommand TankDrive::DriveForwardCmd(Feedback &fb, double dist,
                                       vex::directionType dir, double max_speed,
                                       double end_speed) {
    return DriveForwardCommand(*this, fb, dist, dir, max_speed, end_speed);
}

AutoCommand TankDrive::TurnToHeadingCmd(double heading, double max_speed,
                                        double end_speed) {
    return TurnToHeadingCommand(*this, *turn_default_feedback, heading,
                                max_speed, end_speed);
}
AutoCommand TankDrive::TurnToHeadingCmd(Feedback &fb, double heading,
                                        double max_speed, double end_speed) {
    return TurnToHeadingCommand(*this, fb, heading, max_speed, end_speed);
}

AutoCommand TankDrive::TurnDegreesCmd(double degrees, double max_speed,
                                      double end_speed) {
    return TurnDegreesCommand(*this, *turn_default_feedback, degrees, max_speed,
                              end_speed);
}
AutoCommand TankDrive::TurnDegreesCmd(Feedback &fb, double degrees,
                                      double max_speed, double end_speed) {
    return TurnDegreesCommand(*this, fb, degrees, max_speed, end_speed);
}
AutoCommand TankDrive::PurePursuitCmd(PurePursuit::Path path, directionType dir,
                                      double max_speed, double end_speed) {
    return PurePursuitCommand(*this, *drive_default_feedback, path, dir,
                              max_speed, end_speed);
}
AutoCommand TankDrive::PurePursuitCmd(Feedback &feedback,
                                      PurePursuit::Path path, directionType dir,
                                      double max_speed, double end_speed) {
    return PurePursuitCommand(*this, feedback, path, dir, max_speed, end_speed);
}
...
void TankDrive::drive_tank(double left, double right, int power, BrakeType bt) {
    ...
    bool should_brake = (bt != BrakeType::None) &&
                        fabs(left) < brake_threshold &&
    ...
    if (bt == BrakeType::ZeroVelocity) {
        ...
        if (fabs(vel) <= 0.01 && !captured_position) {
            ...
            double dist_to_target =
                odometry->pos_diff(target_pose, odometry->get_position());
...
bool TankDrive::drive_forward(double inches, directionType dir,
                              Feedback &feedback, double max_speed,
                              double end_speed) {
 ...
 */
bool TankDrive::turn_degrees(double degrees, Feedback &feedback,
                             double max_speed, double end_speed) {
    // We can't run the auto drive function without odometry
    if (odometry == NULL) {
...
bool TankDrive::drive_to_point(double x, double y, vex::directionType dir,
    ...
    if (dir == directionType::fwd) {
        ...
        delta_heading =
            OdometryBase::smallest_angle(current_pos.rot - 180, heading);
    }
...
bool TankDrive::drive_to_point(double x, double y, vex::directionType dir,
                               double max_speed, double end_speed) {
    if (drive_default_feedback != NULL) {
        return this->drive_to_point(x, y, dir, *drive_default_feedback,
                                    max_speed, end_speed);
    }
 ...
 */
bool TankDrive::turn_to_heading(double heading_deg, Feedback &feedback,
                                double max_speed, double end_speed) {
    // We can't run the auto drive function without odometry
    if (odometry == NULL) {
        fprintf(stderr, "Odometry is NULL. Unable to run drive_forward()\n");
    ...
    if (!func_initialized) {
        double initial_delta = OdometryBase::smallest_angle(
            odometry->get_position().rot, heading_deg);
        feedback.init(-initial_delta, 0, odometry->get_angular_speed_deg(),
                      end_speed);
    ...
    double delta_heading =
        OdometryBase::smallest_angle(odometry->get_position().rot, heading_deg);
 ...
 */
bool TankDrive::pure_pursuit(PurePursuit::Path path, directionType dir,
                             Feedback &feedback, double max_speed,
                             double end_speed) {
    ...
    point_t lookahead = PurePursuit::get_lookahead(
        points, odometry->get_position(), path.get_radius());
    ...
    double correction = 0;
    double dist_remaining = PurePursuit::estimate_remaining_dist(
        points, robot_pose, path.get_radius());
    double angle_diff = 0;

    // Robot is facing forwards / backwards, change the bot's angle by 180
    if (dir != directionType::rev) {
        angle_diff = OdometryBase::smallest_angle(
            robot_pose.rot, rad2deg(atan2(localized.y, localized.x)));
    } else {
        angle_diff = OdometryBase::smallest_angle(
            robot_pose.rot + 180, rad2deg(atan2(localized.y, localized.x)));
    }

    // Correct the robot's heading until the last cut-off
    if (!(is_last_point && robot_pose.get_point().dist(last_point) <
...
bool TankDrive::pure_pursuit(PurePursuit::Path path, directionType dir,
                             double max_speed, double end_speed) {
    return pure_pursuit(path, dir, *config.drive_feedback, max_speed,
                        end_speed);
}